# diario_faculdade.py (DIÁRIO PARA FACULDADE - NOTAS P1/P2/P3/PFF - CORRIGIDO SEM AVISO RENDER_DB_URL)
...
def criar_e_popular_sqlite():
    ...
    # SOLUÇÃO PARA O ERRO 'no column named nome_completo'
    try:
...
def inserir_nota_no_db(id_aluno, id_disciplina, tipo_avaliacao, valor_nota):
    if valor_nota is None or valor_nota < 0 or valor_nota > 10.0:
        ...
        return
    # Valida se o tipo de avaliação é P1, P2 ou P3
...
def main():
            ...
            if is_admin:
                ...
                if data_expiracao_str:
                    ...
                    if data_hoje <= data_expiracao:
                        ...
                        st.session_state.is_restricted = True
                        is_expired = True
                else:
                    st.session_state.is_restricted = True
    ...
    if st.session_state.user_login_name is not None and not submitted:
        ...
        if user_data_reloaded:
            ...
            if is_admin:
                st.sidebar.success(f"Login bem-sucedido! Bem-vindo, {nome_exibicao} (Admin).")
            elif is_expired:
            ...
            if is_admin:
                ...
                # O bloco de aviso 'RENDER_DB_URL não configurada' FOI REMOVIDO AQUI
